...
class RoomListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        try:
            ...
            # Chỉ lấy phòng có ít nhất 1 phòng trống
            queryset = queryset.filter(available_rooms__gt=0)

            # ✅ Nếu truyền start_date & end_date → lọc theo ngày trong Room
            if start_date and end_date:
                sd = datetime.strptime(start_date, "%Y-%m-%d").date()
                ed = datetime.strptime(end_date, "%Y-%m-%d").date()
                if sd <= ed:
                ...
                # ✅ Nếu không truyền ngày → lấy tất cả phòng khả dụng, KHÔNG lọc theo today
                pass
...
class RoomSearchView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if start_date and end_date:
            sd = datetime.strptime(start_date, "%Y-%m-%d").date()
            ed = datetime.strptime(end_date, "%Y-%m-%d").date()
            queryset = queryset.filter(start_date__lte=sd, end_date__gte=ed)
